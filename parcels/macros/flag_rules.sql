{% macro flag_rules() %}
    {#  Ordered; list order IS the CASE precedence. Each predicate is a boolean
        expression over the columns of the POST-REPAIR relation (int_parcels_repaired) and enriched with validation.
        Judging happens in int_parcels_validated; this macro only declares. #}
    {% set rules = [
        {'rule_id': 'area_mismatch',    'description': 'geometric area in acres does not match source reported acreage (within tolerance for projection)',       'predicate': 'abs(area_ratio - 1) > {% var(area_tolerance) %}'},
